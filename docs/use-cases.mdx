...
<Tabs>
  ...
  <TabItem value="nodejs-script" label="Node.js Script">
    ...
    const API_URL = 'https://chainx-api-a154bfdeaf7a.herokuapp.com/api/v1/scan';
    ...
    async function scanAllContracts() {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (result.securityScore?.summary?.critical > 0) {
            console.error(`❌ Critical vulnerabilities in ${file}`);
            process.exit(1);
          } else {
            console.log(`✅ ${file} - Score: ${result.securityScore?.score}`);
          }
        } catch (error) {
      ...
      console.log('✅ All contracts scanned successfully');
    }
  ...
  <TabItem value="python-script" label="Python Script">
    ...
    CONTRACTS_DIR = './contracts'
    API_URL = 'https://chainx-api-a154bfdeaf7a.herokuapp.com/api/v1/scan'
    ...
    def scan_all_contracts():
        ...
        for file_path in contract_files:
            ...
            try:
                ...
                if result.get('securityScore', {}).get('summary', {}).get('critical', 0) > 0:
                    print(f'❌ Critical vulnerabilities in {file_name}')
                    has_critical = True
                else:
        ...
        print('✅ All contracts scanned successfully')
...
- 💬 Discord: [Join Community](https://discord.gg/chainx)
- 🐙 GitHub: [View Examples](https://github.com/chainx-scanner/examples)
