...
<Tabs>
  ...
  <TabItem value="javascript" label="JavaScript">
    ...
    async function aiScanContract(filePath, apiKey, openrouterKey, model) {
      ...
      const response = await fetch('https://chainx-api-dev-70e077384a6e.herokuapp.com/api/v1/ai-scan', {
        method: 'POST',
        headers: {
          'X-API-Key': apiKey
        },
  ...
  <TabItem value="python" label="Python">
    ...
    def ai_scan_contract(file_path, api_key, openrouter_key, model):
        url = 'https://chainx-api-dev-70e077384a6e.herokuapp.com/api/v1/ai-scan'
        headers = {'X-API-Key': api_key}
